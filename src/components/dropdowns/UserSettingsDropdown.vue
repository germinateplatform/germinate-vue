<template>
  <b-nav-item-dropdown right v-if="storeServerSettings && storeServerSettings.authMode !== 'NONE'">
    ...
    <template v-if="storeToken && userIsAtLeast(storeToken.userType, USER_TYPE_DATA_CURATOR)">
      ...
      <b-dropdown-item :to="{ name: Pages.backup }" v-if="userIsAtLeast(storeToken.userType, USER_TYPE_ADMINISTRATOR)"><span class="text-warning"><MdiIcon :path="mdiBackupRestore"/></span> {{ $t('dropdownUserSettingsBackup') }}</b-dropdown-item>
...
import { mdiAccount, mdiCog, mdiAccountKey, mdiUpload, mdiCircleMultiple, mdiSetMerge, mdiLogoutVariant, mdiLoginVariant, mdiCommentQuoteOutline, mdiTextSearch, mdiBackupRestore } from '@mdi/js'
