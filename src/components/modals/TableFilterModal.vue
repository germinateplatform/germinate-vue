                    ...
                    <div class="d-flex flex-column ga-2">
                      ...
                      <div class="d-flex ga-2">
                        <template v-if="filter.filter.comparator === FilterComparator.inSet">
                          <v-select
                            v-if="filter.column?.dataType === 'traitClass'"
                            :items="traitClassOptions"
                            multiple
                            density="compact"
                            hide-details
                            :disabled="filter.filter.canBeChanged === false"
                          ...
                          />
                          <v-select
                            v-else-if="filter.column?.dataType === 'methodClass'"
                            :items="methodClassOptions"
                            multiple
                            density="compact"
                            hide-details
                            :disabled="filter.filter.canBeChanged === false"
                        ...
                        >
                          ...
                          />
                          <v-select
                            v-else-if="filter.column?.dataType === 'traitClass'"
                            :items="traitClassOptions"
                            density="compact"
                            hide-details
                            :disabled="filter.filter.canBeChanged === false"
...
<script setup lang="ts">
  ...
  const methodClassOptions = computed(() => {
    return Object.keys(methodClasses).map(e => {
      return {
        value: e,
        title: methodClasses[e].text(),
      }
    })
  })

  const traitClassOptions = computed(() => {
    return Object.keys(traitClasses).map(e => {
      return {
        value: e,
        title: traitClasses[e].text(),
      }
    })
  })
  ...
  function getValidComps (dataType: string | undefined) {
    if (dataType) {
      return (validCompsForType[dataType] || [FilterComparator.equals, FilterComparator.isNull, FilterComparator.isNotNull]).map((v: string) => comps.value.find(c => c.id === v))
    } else {
      return undefined
    }
  }
  ...
  function setInitialComparator (filter: InternalFilter) {
    if (filter.column && filter.column.dataType) {
      filter.filter.comparator = validCompsForType[filter.column.dataType].find(c => c === FilterComparator.contains) || validCompsForType[filter.column.dataType][0]
